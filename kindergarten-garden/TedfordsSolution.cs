...
//from http://exercism.io/submissions/6cd6a0d409be4398a8f07d46d6272838
...
public class Garden
{
    ...
    public Garden(IEnumerable<string> children, string windowSills)
    {
        ...
        _plants = children.OrderBy(i => i)
            ...
            {
                Child = c,
                Plants = rows.SelectMany
                    (r => GetPlant(r, i * 2))
                    .Where(e => e != default(char))
                    .Select(p => Lookup[p])})
    ...
    public static Garden DefaultGarden(string windowSills)
        => new Garden(new[] { "Alice", "Bob", "Charlie", "David", "Eve", "Fred", "Ginny", "Harriet", "Ileana", "Joseph", "Kincaid", "Larry" }, windowSills);
}
